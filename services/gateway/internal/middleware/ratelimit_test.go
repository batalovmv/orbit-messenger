...
// setupRateLimitApp creates a Fiber app with rate limiting and verifies Redis is reachable.
// If Redis is not available, the test is skipped.
func setupRateLimitApp(t *testing.T, maxPerMin int, keyPrefix string) (*fiber.App, *redis.Client) {
	t.Helper()
...
func TestRateLimit_AllowsUnderLimit(t *testing.T) {
	const limit = 5
	app, _ := setupRateLimitApp(t, limit, "test_under")
...
func TestRateLimit_BlocksOverLimit(t *testing.T) {
	const limit = 5
	app, _ := setupRateLimitApp(t, limit, "test_over")
...
func TestRateLimit_ReturnsRetryAfter(t *testing.T) {
	const limit = 2
	app, _ := setupRateLimitApp(t, limit, "test_retry")
...
func TestRateLimit_DifferentKeys(t *testing.T) {
	const limit = 3
	app, _ := setupRateLimitApp(t, limit, "test_keys")
	...
	for i := 0; i < limit; i++ {
		...
		if resp.StatusCode != http.StatusOK {
			t.Fatalf("user-B request %d: expected 200, got %d (user-A's limit should not affect user-B)", i+1, resp.StatusCode)
		}
	}
}
