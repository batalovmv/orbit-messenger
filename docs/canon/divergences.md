...
| TZ-ORBIT-MESSENGER.md | Глобальные роли `superadmin` / `compliance` | **Реализовано частично.** 4 системные роли (`superadmin` / `compliance` / `admin` / `member`) с битмаской в [pkg/permissions/system.go](../../pkg/permissions/system.go); используется backend (admin export, audit log, feature flags, maintenance) и фронтом (AdminPanel, CompliancePanel) с feature-gate по роли. Отдельный bootstrap-хелпер `superadmin_bootstrap` отключён в state.json `removed_features` — это про автоматическое создание первого superadmin'а, не про саму роль; 4-ролевая модель живая. |
