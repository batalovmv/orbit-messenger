...
- **Feature flags**: `pkg/featureflags/registry.go` — единый реестр известных ключей с уровнем экспозиции (`unauth/auth/admin/server_only`), safety class и UX-флагом `Dangerous`. Админский CRUD: `PATCH /api/v1/admin/feature-flags/:key` (gate `SysManageSettings`, audit fail-closed). Клиент полит `/system/config` (auth) и `/public/system/config` (без JWT) с TTL-кэшем 30s в messaging. AdminPanel Flags tab: segmented control по `exposure` (client-side фильтр над in-code registry), per-row кнопка «История» открывает modal с reuse-фильтром `target_type=feature_flag&target_id=<key>` поверх существующего `/admin/audit-log`, и confirm-modal перед toggle-ON для `Dangerous` флагов (сейчас: `e2e_dm_enabled`, `calls_group_enabled`). Toggle disabled для `!known` строк — backend `Set()` всё равно отверг бы их с 400 «Unknown feature flag».
- **Maintenance banner**: одна строка в `feature_flags(maintenance_mode)`. Метаданные `{message, block_writes, since, updated_by, start_at?, end_at?}`. Gateway middleware (`services/gateway/internal/middleware/maintenance.go`) при `block_writes=true` отклоняет POST/PUT/PATCH/DELETE с 503+`Retry-After` для не-superadmin; superadmin проходит сквозь. Веб-баннер рисуется через `MaintenanceBanner.tsx` поверх Main. **Scheduled mode**: опциональный временной window (`start_at` / `end_at`) — backend `Maintenance()` вычисляет effective `Active = enabled AND now ∈ [start_at, end_at]` при каждом чтении (`maintenanceWindowOpen`); никаких миграций / sweeper'ов / cron'ов. AdminPanel MaintenanceTab показывает live preview через `MaintenanceBannerView` (отделённый от `withGlobal`-обёртки) и принимает `<input type="datetime-local">` для start_at/end_at. Frontend конвертирует local-time → ISO UTC через `.toISOString()` ДО отправки, чтобы не было TZ-drift round-trip'а; backend handler валидирует window (rejects equal/inverted с 400). Operator metadata (`since`, `updated_by`, `start_at`, `end_at`) НЕ утекает в `PublicMaintenanceState` — locked test покрывает все четыре поля.
- **Audit log search + CSV export**: `audit_store.AuditFilter.Q` — ILIKE по `action / target_type / target_id / actor.display_name / details::text / ip`. Backslash/`%`/`_` экранируются. На 150-user pilot хватает; pg_trgm/tsvector — отложено. AdminPanel AuditTab держит dropdown-фильтры по `action` (whitelist `model.AuditActions()`) / `target_type` (whitelist `model.AuditTargetTypes()`) и `<input type="date">` для `since`/`until` — все AND-комбинируются с free-text q. `GET /admin/audit-log/export` стримит CSV до `AuditExportHardCap=200_000` строк (≈3 месяца audit'а на pilot; cap упирается в gateway-buffer `doProxy`). Gated `SysViewAuditLog AND SysExportData` — admin читает в UI, экспортит только compliance/superadmin (separation of duties). `AdminService.PreflightAuditExport` делает все RBAC-чеки + actor-rank pivot guard + audit-first `audit.export` row синхронно ДО `SetBodyStreamWriter` — любая ошибка возвращает реальный 4xx/5xx, не 200-CSV-with-#error. CSV cells проходят через `csvSafe()`: формулы `=`/`+`/`-`/`@`/TAB/CR defang'ятся ведущим `'` (OWASP CSV Injection). Тесты: handler happy-path / RBAC reject / unknown-action / fail-closed / formula-injection.
