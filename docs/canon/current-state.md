...
- **X-App-Version handshake** (PR #20): web client шлёт `X-App-Version: <pkg.json:version>` на каждом API-запросе; gateway middleware (`services/gateway/internal/middleware/app_version.go`) добавляет `X-App-Latest-Version` в каждый ответ когда `APP_LATEST_VERSION` env задан. Если задан `APP_MIN_VERSION` AND клиентская версия ниже → 426 Upgrade Required + `X-Required-Version` + `Retry-After: 30`. Skip для `/api/v1/auth/*` и `/api/v1/public/*` (логин и polling должны работать). Опционально оба env'а — пустые = no-op (не блокируют ничего). Клиент при mismatch / 426 dispatch'ит `checkAppVersion({force: true})` — баннер обновления появляется на первом API-вызове после деплоя, без ожидания периодического `version.txt`-poll.
