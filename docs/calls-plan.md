...
**Partial completion:**
- useSfuStreamManager hook created (`web/src/hooks/useSfuStreamManager.ts`)
- connectToActiveGroupCall fixed to detect Saturn SFU vs legacy Colibri
- Auto-routing implemented in requestCall (member_count > 2 → group mode)

**What remains:**
- Full GroupCall.tsx integration with useSfuStreamManager hook
- Video grid mapping verification
- 3-window live test with real participants
