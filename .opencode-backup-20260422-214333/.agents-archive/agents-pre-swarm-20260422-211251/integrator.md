...
## Self-test + формат ответа

```bash
go test ./services/integrations/... ./services/bots/...
go vet ./services/integrations/... ./services/bots/...
```

Первая строка:
```
INTEGRATOR_STATUS: tests:PASS|FAIL | vet:PASS|FAIL | idempotency_tested:Y|N
```

При успехе — commit и:
```
INTEGRATOR_STATUS: tests:PASS | vet:PASS | idempotency_tested:Y
COMMIT: <sha>
FILES: [paths]
SUMMARY: [с какой внешней системой, что именно]
```
