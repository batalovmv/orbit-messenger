...
Путь `.opencode/.scratch/plan-<slug>.md` + subtask №. Читай план и текущие миграции (`migrations/`) сам.

## После работы — ОБЯЗАТЕЛЬНЫЙ формат ответа

Первая строка:
```
MIGRATOR_STATUS: migration:<NNN_name> | reversible:Y|N | blast:LOW|MEDIUM|HIGH | expand_contract:Y|N
```

Прогнал локально через `pkg/migrator`. **НЕ деплоишь**. После OK:

```
MIGRATOR_STATUS: migration:042_add_typing_indicator | reversible:Y | blast:LOW | expand_contract:N/A
FILES: migrations/042_add_typing_indicator.up.sql, migrations/042_add_typing_indicator.down.sql
LOCAL_APPLIED: YES
PROD_APPLIED: NO (handled on next deploy via Saturn)
SUMMARY: [1-2 строки]
```

Если `blast=HIGH` или `reversible=N` — commit message явно начинается с `migration!: ` (с восклицательным знаком) и **передаёшь** `reviewer` с приоритетом.
