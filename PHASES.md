...
- [~] Глубокая интеграция existing TG Web A `GroupCall.tsx` UI с Saturn SFU streams (E3.F4) — адаптер создан (`useSfuStreamManager`), integration в процессе. Требует QA
- [~] Live-тест 3 окнами Chrome incognito по сценариям 11-13 из docs/calls-plan.md — требует ручного прогона
- [x] Auto-routing P2P vs SFU на frontend (member_count > 2 → mode='group') — реализовано в `requestCall`
...
## Phase 7: E2E Encryption — **REVERTED** (2026-04-15)

**Статус:** Полностью откачено. Конфликт с корпоративной моделью доступа.

**Причина отката:** Orbit — корпоративный мессенджер MST. Compliance/superadmin-роли
RBAC ДОЛЖНЫ иметь возможность читать любую переписку, включая soft-deleted. True
E2E (Signal, X3DH + Double Ratchet) криптографически исключает это: ключи живут
на устройствах юзеров, сервер физически не может расшифровать. Key escrow
(ghost-recipient) — скрытый backdoor, юридически скользко и архитектурно хрупко.

**Что вместо этого (new Phase 8D подраздел — «at-rest encryption»):**

- AES-256-GCM обёртка `messages.content` на уровне store-слоя
  ([services/messaging/internal/store/message_store.go](services/messaging/internal/store/message_store.go))
  — сервер шифрует перед INSERT, расшифровывает при SELECT. Прозрачно для
  service/handler слоёв.
- Мастер-ключ в env: `ORBIT_MESSAGE_ENCRYPTION_KEY` (30+ chars, SHA-256→AES-256).
- Кража дампа БД без ключа → только мусор. Admin панель через обычный service
  layer читает всё прозрачно.
- Soft-delete дисциплина: `messages.is_deleted = true` + `content = NULL`,
  строка остаётся в БД навсегда для compliance-аудита (COMMENT задокументирован).
- Медиа at-rest encryption — **TODO Phase 8D** (per-file ключ, мастер-ключ
  оборачивает его; media service расшифровывает на download).

**Что было удалено (rollback commit):**

- Фронтенд: `web/src/lib/crypto/*` (18 файлов, ~3200 LOC), client-side search
  index, Saturn `keys`/`encryptedMessages` методы, `SafetyNumbersModal`,
  `DisappearingTimerListItem`, `@noble/*` deps.
- Бэкенд: `SendEncryptedMessage` handler/service, `CreateEncrypted`/
  `CreateEncryptedWithMedia` store методы, `SetDisappearingTimer`/`SetIsProtected`
  endpoints, поля `IsEncrypted`/`EncryptedContent`/`DisappearingTimer`/
  `IsProtected`/`ExpiresAt` из моделей messaging, Phase 7.1 coupling в media.
- Миграция: [053_rollback_phase7_add_at_rest.sql](migrations/053_rollback_phase7_add_at_rest.sql)
  дропает таблицы `user_keys`, `one_time_prekeys`, `key_transparency_log`,
  `compliance_keys` и колонки `chats.is_encrypted/disappearing_timer/is_protected`,
  `messages.encrypted_content/expires_at`, `media.is_encrypted`.

**Deferred follow-up (не входит в rollback, будет в Phase 8D):**

- Очистка `is_encrypted` поля в media service (сейчас оставлено как
  isolated dead code — не ломает сборку).
- Удаление устаревших хендлер/сервис тестов с моками E2E методов.
- At-rest encryption для R2 blobs.
- KMS интеграция (сейчас env-key).
- Admin-audit log: запись в `audit_log` при каждом чтении чужих чатов
  compliance/superadmin юзером.
- ToS обновление: «администрация имеет доступ к переписке».

---

## Phase 7 (ORIGINAL — for history): E2E Encryption — СДЕЛАНО И ОТКАЧЕНО

**Цель (изначальная):** Zero-Knowledge — сервер не может прочитать DM. Криптографическая гарантия.
...
- [x] **8C Integrations:** Изучить API InsightFlow, Keitaro, Saturn.ac — форматы webhook payload (framework-only presets реализованы, live credentials pending)
- [x] **8C Integrations:** Спроектировать generic webhook framework — verification (HMAC), retry, logging
- [x] **8D ScyllaDB:** Спроектировать migration strategy — OVERKILL для 150 пользователей, PostgreSQL достаточно
- [x] **8D ScyllaDB:** Спроектировать CQL schema — OVERKILL, отложено
- [x] **8D NATS:** Спроектировать stream topology — 1 stream ORBIT с orbit.> wildcard, 24h retention (реализовано)
- [x] **8D Monitoring:** Выбрать: Prometheus+Grafana managed (Saturn?) или self-hosted? — Saturn.ac managed достаточно для пилота
- [x] **8D Security:** Составить OWASP checklist для аудита всех сервисов — OWASP Top 10 sweep выполнен
- [x] Продумать: backup strategy — pg_dump каждые 4h + Saturn.ac hourly backups (RPO=1h), WAL deferred, R2 cross-region OVERKILL
...
- [x] sendPhoto, sendDocument, sendVideo, sendAudio, sendVoice (multipart upload → media service → messaging)
...
> **Статус**: Local verification passed (2026-04-22). Для пилота на 150 юзеров: ScyllaDB/NATS/Redis уже готовы. Оставшееся — pre-GA hardening.

**ScyllaDB миграция:**
- [ ] ~~Messages table → ScyllaDB~~ — OVERKILL для 150 юзеров, PostgreSQL достаточно
- [ ] ~~Dual-write стратегия миграции~~ — OVERKILL
- [ ] ~~Target: 1000 msg/sec~~ — OVERKILL

**NATS JetStream:**
- [x] NATS работает как 1 stream `ORBIT` под `orbit.>` с 24h retention (done, отложено split на 4 stream — future optimization)
- [x] Publisher/subscriber wired, dedup по Nats-Msg-Id

**Redis:**
- [x] `jwt_cache:*`, `jwt_blacklist:*`, `ratelimit:*` — работают (done)
- [x] `online`/`typing` — in-memory Gateway Hub (корректный дизайн для WS)

**Мониторинг:**
- [x] Prometheus — RPS + latency histogram per service/method/route/status, WS active connections gauge. `pkg/metrics` shared wrapper, /metrics on every service gated by X-Internal-Token. Optional local `monitoring` compose profile scrapes all 8 services (commit `ebd1689`, Saturn uses managed stack)
- [x] Grafana — local-dev dashboards ready (SLO Overview + Service Health), **deferred to Phase 8D.x** (Saturn.ac managed metrics sufficient for pilot, own Grafana when per-project monitoring available)
- [ ] ~~OpenTelemetry distributed tracing~~ — OVERKILL, Prometheus+Grafana достаточно
- [x] External healthcheck каждые 30 сек (`scripts/healthcheck.sh`)
- [x] Alerts → Orbit канал "MST Monitoring" (dogfooding) — Alertmanager → integrations preset `alertmanager`

**Live Translate / Мультиязычность:**
- [x] Migration 056: live translate settings and cache (applied)
- [x] Migration 058: translate preferences columns (applied)
- [x] Frontend wiring: `loadSaturnSettings` action загружает `can_translate`/`can_translate_chats` из backend после sync (actions.ts, settings.ts, sync.ts)
- [ ] Проверка на проде (**TODO 8D.4**: см. `docs/8d-qa-checklist.md`)

**Phase 6: Calls QA (browser test):**
- [ ] P2P звонок — проверить 1-на-1 в разных браузерах
- [ ] Групповой звонок (SFU) — проверить 3+ участников
- [ ] Screen sharing — проверить start/stop
- [ ] Push уведомления на звонок когда app закрыт (**TODO 8D.5**)
- [ ] AI verification by Claude Sonnet (see `docs/8d-qa-checklist.md`) (**TODO 8D.6**)

**Security audit:**
- [x] Rate limiting ALL endpoints: edge 20/min for /ai/* in gateway (already enforced per-user inside ai service), WS pre-auth 60/min per IP (Saturn ingress shares IPs)
- [x] Integrations webhook fail-closed when connector secret is empty + timestamp required alongside signature
- [x] Messaging GetMessage returns 404 uniformly (no IDOR oracle distinguishing 403/404)
- [x] Chat list at-rest decrypt, scheduled messages encrypt-on-write (previously leaking ciphertext/plaintext)
- [x] go vet (8 services) — passed
- [x] Tests + BOM fix (translation_store.go, clamav.go)
- [x] Security sweep — IDOR protection, rate limiting, auth middleware verified
- [x] govulncheck — clean (0 vulnerabilities)
- [x] CORS whitelist — verified, no `*` + credentials
- [x] BUG-04: media malware audit fail-closed
- [x] Docker healthchecks — infra OK, app /health exists; HEALTHCHECK added to all 8 service Dockerfiles (auth/messaging/media/calls/ai/bots/integrations + gateway)
- [x] OWASP Top 10 full sweep — A01 (no IDOR), A02 (no MD5/SHA1 except RFC7635 TURN), A03 (no SQL injection via fmt.Sprintf), A05 (no CORS wildcard), A07 (auth middleware on all routes), A09 (no secrets in logs), A10 (SSRF protection on webhook URLs); SAST 0 findings
- [x] Input validation — systematic pass (ai/calls/bots/integrations handlers: UUID, string length, enum, pagination caps, webhook_url HTTPS+ParseRequestURI, config 16KB, rune-based field caps)
- [x] Secrets rotation — документация написана (`docs/secrets-rotation.md`): процедуры для всех 14 секретов, расписание ротации, чеклист при компрометации
...
- [x] Admin toolkit polish C — Maintenance scheduled mode (optional `start_at`/`end_at` in metadata, evaluated at read time via `maintenanceWindowOpen`; handler rejects equal/inverted windows with 400; sanitiser drops them as defence-in-depth) + live `MaintenanceBannerView` preview in MaintenanceTab. TZ-correct round-trip: client sends `.toISOString()` (UTC), server stores RFC3339; operator metadata leak guard now also covers `start_at`/`end_at`.
...
- [x] WAL archiving — deferred, hourly backups sufficient for pilot
- [ ] ~~ScyllaDB: snapshot + incremental~~ — OVERKILL (не на ScyllaDB)
- [x] Redis: RDB snapshot каждые 15 мин + AOF everysec (docker-compose redis command)
- [ ] ~~R2: cross-region replication~~ — OVERKILL, R2 имеет встроенную durability
- [ ] E2E keys: client-side encrypted backup (user-initiated) — обсуждать когда добавим E2E

**Scaling roadmap (150 → 500 → 1000+):**
- [ ] 150 users: single PostgreSQL, single Redis, single gateway instance
- [ ] ~~500 users: PostgreSQL read replicas, Redis Sentinel, ScyllaDB, 2-3 gateway instances + LB~~ — OVERKILL пока
- [ ] ~~1000+ users: sharded PostgreSQL, Redis Cluster, ScyllaDB multi-DC, auto-scaling gateway~~ — OVERKILL пока
...
- [ ] ~~gRPC миграция~~ — OVERKILL, HTTP достаточно даже при 500+
