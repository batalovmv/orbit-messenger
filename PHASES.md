...
**Мониторинг:**
- [x] Prometheus — RPS + latency histogram per service/method/route/status, WS active connections gauge. `pkg/metrics` shared wrapper, /metrics on every service gated by X-Internal-Token. docker-compose Prometheus scrapes all 8 services (commit `ebd1689`, Saturn uses managed stack)
