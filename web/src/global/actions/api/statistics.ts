...
addActionHandler('loadStatistics', async (global, actions, payload): Promise<void> => {
  ...
  if (stats.type === 'channel') {
    const messageInteractions = stats.recentPosts.filter(
      (post: StatisticsMessageInteractionCounter | StatisticsStoryInteractionCounter) => post.type === 'message',
    );
    const storyInteractions = stats.recentPosts.filter(
      (post: StatisticsMessageInteractionCounter | StatisticsStoryInteractionCounter) => post.type === 'story',
    );
    ...
    if (messageInteractions.length > 0) {
      actions.loadMessagesById({
        ...
        messageIds: messageInteractions.map(
          (interaction: StatisticsMessageInteractionCounter | StatisticsStoryInteractionCounter) => (interaction as StatisticsMessageInteractionCounter).msgId,
        ),
      });
    }
    ...
    if (storyInteractions.length > 0) {
      actions.loadPeerStoriesByIds({
        peerId: chatId,
        storyIds: storyInteractions.map(
          (interaction: StatisticsMessageInteractionCounter | StatisticsStoryInteractionCounter) => (interaction as StatisticsStoryInteractionCounter).storyId,
        ),
      });
    }
  }
