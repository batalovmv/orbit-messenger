...
addActionHandler('loadMyUniqueGifts', async (global, actions, payload): Promise<void> => {
  ...
  const byId = buildCollectionByCallback(gifts, (savedGift: ApiSavedStarGift) => (
    [savedGift.gift.id, savedGift]
  ));

  const ids = gifts.map((gift: ApiSavedStarGift) => gift.gift.id);
...
addActionHandler('openGiftUpgradeModal', async (global, actions, payload): Promise<void> => {
  ...
  const filteredPrices = preview.prices.filter((price: ApiStarGiftUpgradePrice) => price.date > serverTime);
  const filteredNextPrices = preview.nextPrices.filter((price: ApiStarGiftUpgradePrice) => price.date > serverTime);

  const passedPrices = preview.nextPrices.filter((price: ApiStarGiftUpgradePrice) => price.date <= serverTime);
...
addActionHandler('loadActiveGiftAuctions', async (global, actions, payload): Promise<void> => {
  ...
  global = getGlobal();
  result.auctions.forEach((auction: ApiStarGiftAuctionState) => {
    global = replaceGiftAuction(global, auction);
  });
  global = {
    ...global,
    activeGiftAuctionIds: result.auctions.map((auction: ApiStarGiftAuctionState) => auction.gift.id),
  };
  setGlobal(global);
});
  ...
  if (regularGiftId) {
    ...
    if (result) {
      const craftableModels = result.attributes.filter(
        (attr: ApiStarGiftAttribute) => attr.type === 'model' && attr.rarity.type !== 'regular',
      );
...
addActionHandler('openGiftCraftSelectModal', async (global, actions, payload): Promise<void> => {
  ...
  const savedGifts = myGiftsResult?.gifts.filter((g: ApiSavedStarGift) => g.gift.type === 'starGiftUnique');
  const marketGifts = marketGiftsResult?.gifts.filter(
    (g: ApiStarGift): g is ApiStarGiftUnique => g.type === 'starGiftUnique',
  );
...
addActionHandler('loadMoreCraftableGifts', async (global, actions, payload): Promise<void> => {
  ...
  // Filter to only unique gifts
  const newSavedGifts = result.gifts.filter((g: ApiSavedStarGift) => g.gift.type === 'starGiftUnique');
...
addActionHandler('loadMoreMarketCraftableGifts', async (global, actions, payload): Promise<void> => {
  ...
  const newGifts = result.gifts.filter((g: ApiStarGift): g is ApiStarGiftUnique => g.type === 'starGiftUnique');
