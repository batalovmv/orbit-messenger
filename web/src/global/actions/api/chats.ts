...
addActionHandler('loadAllChats', async (global, actions, payload): Promise<void> => {
  ...
  while (!global.chats.isFullyLoaded[listType]) {
    ...
    if (result?.threadInfos) {
      result.threadInfos.forEach((threadInfo: ApiThreadInfo) => {
...
addActionHandler('markChatMessagesRead', async (global, actions, payload): Promise<void> => {
  ...
  while (hasMoreTopics) {
    ...
    result.topics.forEach((topicWithState: ApiTopicWithState) => {
...
addActionHandler('loadGroupsForDiscussion', async (global): Promise<void> => {
  ...
  const addedById = groups.reduce((result: Record<string, ApiChat>, group: ApiChat) => {
...
addActionHandler('toggleJoinToSend', async (global, actions, payload): Promise<void> => {
  ...
  const chat = selectChat(global, chatId);
  if (!chat) return;
  if (!isChatSuperGroup(chat)) return;
...
addActionHandler('loadTopics', async (global, actions, payload): Promise<void> => {
  ...
  global = getGlobal();
  global = addMessages(global, result.messages);
  result.topics.forEach((topic: ApiTopicWithState) => {
...
addActionHandler('loadBotRecommendations', async (global, actions, payload): Promise<void> => {
  ...
  global = getGlobal();
  global = addUsers(global, users as Record<string, ApiUser>);
...
async function loadChats(
  ...
  global = updateUsers(global, buildCollectionByKey(result.users, 'id') as Record<string, ApiUser>);
  global = updateChats(global, newChats as Record<string, ApiChat>);
  if (isFirstBatch) {
  ...
  if (listType === 'active' || listType === 'archived') {
    ...
    idsToUpdateDraft.forEach((chatId: string) => {
