...
addActionHandler('loadGreetingStickers', async (global): Promise<void> => {
  ...
  global = {
    ...
    stickers: {
      ...
      greeting: {
        hash: greeting.hash,
        stickers: greeting.stickers.filter((sticker: ApiSticker) => sticker.emoji === '👋'),
      },
    },
  };
  setGlobal(global);
...
addActionHandler('loadDefaultTopicIcons', async (global): Promise<void> => {
  ...
  const fullSet = { ...(set as ApiStickerSet), stickers };
...
addActionHandler('loadGenericEmojiEffects', async (global): Promise<void> => {
  ...
  global = {
    ...global,
    genericEmojiEffects: { ...(set as ApiStickerSet), stickers },
  };
  setGlobal(global);
});
...
addActionHandler('setStickerSearchQuery', (global, actions, payload): ActionReturnType => {
  ...
  if (query) {
    void searchThrottled(async () => {
      ...
      const resultIds = result.sets.map(({ id }: { id: string }) => id);
...
addActionHandler('loadFeaturedEmojiStickers', async (global): Promise<void> => {
  ...
  global = {
    ...
    customEmojis: {
      ...global.customEmojis,
      featuredIds: featuredStickers.sets.map(({ id }: { id: string }) => id),
      byId: {
        ...global.customEmojis.byId,
        ...buildCollectionByKey(featuredStickers.sets.flatMap((set: ApiStickerSet) => set.stickers || []), 'id'),
      },
    },
    stickers: {
      ...global.stickers,
