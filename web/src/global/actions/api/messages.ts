...
import { callApi, cancelApiProgress } from '../../../api/saturn';
import {
  getIsSavedDialog,
  getUserFullName,
    ...
    pollResults: {
      ...
      voters: {
        ...
        [option]: unique([
          ...
          ...result.votes.map((vote: { peerId: string }) => vote.peerId),
        ]),
      },
      offsets: {
...
addActionHandler('loadMessageViews', async (global, actions, payload): Promise<void> => {
  ...
  global = getGlobal();
  result.viewsInfo.forEach((update: { id: number; views: number; forwards: number; threadInfo?: ApiThreadInfo }) => {
...
addActionHandler('loadFactChecks', async (global, actions, payload): Promise<void> => {
  ...
  global = getGlobal();
  result.forEach((factCheck: ApiFactCheck, i: number) => {
