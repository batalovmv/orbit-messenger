...
## 071 — call_recordings (2026-05-06)
Compliance recording (Phase D пилотного плана, decisions #1/#2 от 2026-05-06): создана таблица `call_recordings(id, call_id FK→calls CASCADE, participant_user_id FK→users, s3_key, encryption_key_id, started_at, ended_at, duration_sec, size_bytes, upload_failed, created_at)` + два индекса (`idx_call_recordings_call(call_id, started_at)` и partial `idx_call_recordings_retention(ended_at) WHERE ended_at IS NOT NULL`). Audio-only, SFU-only — P2P звонки не пишутся (не идут через сервер). Retention 90 дней управляется D5-кроном по `ended_at`; в этой миграции только схема. `upload_failed` помечает строки, у которых flush из oggwriter прошёл, а S3 upload упал — D2 publisher не ретраит, D5 GC выкидывает их без попытки удалить из S3.
